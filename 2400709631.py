...
class Student:
    ...
    def get_marks(self, subject):
        ...
        return self.std_marks.get(subject,None)
    ...
    def edit_marks(self, subject, new_marks):
        ...
        if subject in self.std_marks and 0<=new_marks<=100:
            self.std_marks[subject]=new_marks
            return True
        return False
...
class Gradebook:
        ...
        try:
            if os.path.exists(self.filename):
                with  open(self.filename,"r") as file:
                    data = json.load(file)
                    for admin_no,std_info in data.items():
                        student=Student(admin_no,std_info['name'])
                        student.std_marks=std_info['marks']
                        self.students[admin_no]=student
        except FileNotFoundError:
            pass    
    
    def save_data(self):
        data ={admin_no:{'name':student.name,'marks':student.std_marks} for admin_no,student in self.students.items()}
        ...
        for subject in subjects:
            ...
            if marks:
                ...
                try:
                    ...
                    grade_stats[f"Mode_Freq_{subject}"]= marks.count(mode(marks))
                except:
                    grade_stats[f'Mode_{subject}']= None
                    grade_stats[f'Mode_Freq_{subject}']= 0
    ...
    def view_student_grades(self, admin_no):
        ...
        student_grades = { subject: student.get_marks(subject) for subject in ["Maths", "SST","English","Science"]}
        if student:
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            admin_no = input("Enter the student's admin number: ")
            ...
            for subject in ["Maths","English","SST","Science"]:
                while True:
                    try:
                        ...
                        print("Invalid input. Please enetr a value between 0 and 100.")
            ...
            admin_no=input("Enter the student's admin number to be deleted: ")
            if gradebook.delete_student(admin_no):
                print(f"Student with {admin_no} has been deleted.")
            else:
                print(f"The {admin_no} not found.")
            gradebook.delete_student(admin_no)
        elif choice == '3':
            stats=gradebook.view_statistics()
            for statistics,value in stats.items():
                print(f"{statistics} : {value}")
               # print("-------------------")


        elif choice == '4':
            admin_no = input("Enter the admin number for the student to be viewed: ")
            ...
            admin_no = input("Enter the number for the student whose marks you want to edit: ")
                ...
                try:
                    if 0<=new_marks<=100:
                        if student.edit_marks(subject, new_marks):
                            ...
                            print(f"Failed to update the marks. Please check the subject and try again!")
                except ValueError:
