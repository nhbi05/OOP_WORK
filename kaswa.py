...
class Student:
    def __init__(self, admin_no, name):
        ...
        self.name = name
        self.marks ={'Maths':None,'Science':None,'SST':None,'English':None}
...
class Gradebook:
        ...
        try:
            if os.path.exists(self.filename):
                    with open(self.filename,"r")as file:
                        data=json.load(file)#always saves the info as a dictionary
    ...
    def view_statistics(self):
        ...
        """
        grade_stats = {}
        subjects =['Maths','Sci','SST','Eng']
        for subject in subjects:
            marks=[student.get_marks(subject)for student in self.students.values() if student.get_marks is not None]
            if marks:
                ...
                grade_stats[f'mode_{subject}']=max(set(marks),key=marks.count)
                grade_stats[f'mode_freq_{subject}']=marks.count(grade_stats[f'mode_{subject}'])
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            admin_no = input("Enter student's admin_no: ")
            name = input("Enter name: ")
            student = Student(admin_no,name)
            for subject in ['Maths','Sci','SST','Eng']:
                while True:
                    marks=int(input(f'Enter mark for{subject}: '))
                    if 0<=marks<=100:
                        student.set_marks(subject,marks)
                        break
                    else:
                        print("Enter mark between 0 and 100")
            if gradebook.add_student(student):
                print('student added')
            else:
                print('student already exists')
        elif choice == '2':
            admin_no = input("Enter admin_no: ")
            if gradebook.delete_student(admin_no):
                print(f"student with {admin_no} deleted. ")
            else:
